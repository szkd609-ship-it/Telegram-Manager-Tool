...
const state = {
  ...
  detail: null,        // selected account phone for detail view
  loginStep: 1,        // 1=phone 2=code 3=2fa
  loginData: {},       // { phone, phoneCodeHash, sessionId }
  selectedPhone: null, // for send/join modals
};
...
function toast(msg, type = 'info') {
  ...
  el.innerHTML = `<span style="font-size:15px;font-weight:800">${icons[type] || 'ℹ'}</span><span>${msg}</span>`;
...
function navigate(page, data) {
  ...
  // Update topbar
  const titles = {
    dashboard: ['Accounts Console', 'Manage all connected Telegram sessions'],
    login: ['Add Account', 'Authenticate a new Telegram account'],
    detail: ['Account Detail', state.detail || ''],
    sendmsg: ['Send Message', 'Send a message from any account'],
    joinchannel: ['Join Channel', 'Join a channel with one or all accounts'],
  };
  ...
  if (page === 'sendmsg') populateAccountSelects();
  if (page === 'joinchannel') populateAccountSelects();
}
...
function renderAccounts(accounts) {
  ...
  const filtered = accounts.filter(a =>
    a.phone.includes(q) || (a.username || '').toLowerCase().includes(q) ||
  ...
  if (!filtered.length) {
    container.innerHTML = `
      <div class="empty-state" style="grid-column:1/-1">
        <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><circle cx="12" cy="8" r="5"/><path d="M3 21a9 9 0 0118 0"/></svg>
        <h3>${accounts.length ? 'No matching accounts' : 'No accounts yet'}</h3>
        <p>${accounts.length ? 'Try a different search' : 'Click "Add Account" to get started'}</p>
      </div>`;
    return;
  }
  ...
  container.innerHTML = filtered.map(a => {
    ...
    return `
      <div class="account-card" onclick="navigate('detail','${a.phone}')" data-testid="card-account-${a.phone}">
...
function populateAccountSelects() {
  ...
  selects.forEach(sel => {
    ...
    sel.innerHTML = `<option value="">-- Select Account --</option>` +
      state.accounts.map(a => `<option value="${esc(a.phone)}">${esc(a.phone)} ${a.firstName ? '('+esc(a.firstName)+')' : ''}</option>`).join('');
...
async function signIn() {
  ...
  try {
    const r = await post('/telegram/sign-in', {
      ...
      sessionId: state.loginData.sessionId,
      password: null,
    });
...
function resetLogin() {
  ...
  if (document.getElementById('login-2fa')) document.getElementById('login-2fa').value = '';
  setLoginStep(1);
}

/* ========== DETAIL ========== */
async function loadDetail(phone) {
  // Render skeleton account header
  ...
  document.getElementById('detail-2fa-badge').textContent = acc.has2fa ? '2FA ON' : '2FA OFF';
  document.getElementById('detail-2fa-badge').className = 'badge ' + (acc.has2fa ? 'badge-yellow' : 'badge-green');

  // Show/hide disable-2fa btn
  ...
  document.getElementById('sessions-list').innerHTML = '<div class="text-dim text-sm">Click "Refresh" to load sessions</div>';
      ...
      el.innerHTML = `
        <div class="code-box">
          <div>
            ...
            <div class="code-meta">From: ${esc(r.from || 'Telegram')} &middot; ${r.date ? new Date(r.date).toLocaleTimeString() : ''}</div>
          </div>
          <button class="btn btn-secondary btn-sm" onclick="copyText('${esc(r.code)}')">Copy</button>
        </div>`;
      toast('Code found: ' + r.code, 'success');
    } else {
      el.innerHTML = `<div class="empty-state" style="padding:16px"><p>No recent login code found</p></div>`;
      toast('No recent login code', 'warn');
    }
  } catch (e) {
    toast('Error: ' + e.message, 'error');
  }
...
function renderSessions(sessions) {
  ...
  if (!sessions || !sessions.length) {
    el.innerHTML = `<div class="empty-state" style="padding:16px"><p>No active sessions found</p></div>`;
    return;
  }
  el.innerHTML = sessions.map(s => `
    <div class="session-item">
      ...
      <div class="session-info flex-1">
        ...
        <div class="session-meta">${esc(s.platform)} &middot; ${esc(s.appName)} &middot; ${esc(s.country)} &middot; Last: ${new Date(s.dateActive).toLocaleDateString()}</div>
      </div>
      ${!s.current ? `<button class="btn btn-danger btn-sm" onclick="terminateSession('${esc(s.hash)}')" data-testid="btn-terminate-${esc(s.hash)}">Kill</button>` : ''}
    </div>`).join('');
}
...
async function logoutAccount() {
  if (!confirm(`Log out ${state.detail}?`)) return;
  try {
    ...
    updateStats(); updateAccountBadge();
    navigate('dashboard');
  } catch (e) {
    toast('Error: ' + e.message, 'error');
  }
...
async function joinAllChannels() {
  ...
  try {
    ...
    el.innerHTML = `
      <div style="margin-top:12px;font-size:12px;font-weight:600;color:var(--text-mid);margin-bottom:8px">RESULTS (${r.results.length} accounts)</div>
      <div class="result-list">
        ${r.results.map(x => `
          <div class="result-item ${x.success ? 'ok' : 'fail'}">
            ...
            <span style="font-family:var(--mono)">${esc(x.phone)}</span>
            ${x.error ? `<span style="color:var(--text-dim);margin-left:auto">${esc(x.error)}</span>` : ''}
          </div>`).join('')}
      </div>`;
